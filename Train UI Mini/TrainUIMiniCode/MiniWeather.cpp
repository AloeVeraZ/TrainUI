...
namespace {
constexpr char URL[] =
  "https://api.open-meteo.com/v1/forecast?latitude=40.7812&longitude=-73.9665"
  "&current=temperature_2m,weather_code,wind_speed_10m,relative_humidity_2m"
  "&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=America%2FNew_York";

bool numberAfter(const String &body, const char *key, float &value) {
  const String needle = String('"') + key + "\":";
  int start = body.indexOf(needle); if (start < 0) return false;
  start += needle.length();
  int end=start;
...
bool MiniWeather::fetch(MiniWeatherSnapshot &result, String &error) {
  ...
  if (!numberAfter(body,"temperature_2m",temperature) || !numberAfter(body,"wind_speed_10m",wind) ||
      !numberAfter(body,"relative_humidity_2m",humidity) || !numberAfter(body,"weather_code",weatherCode)) {
