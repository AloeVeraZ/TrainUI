...
TRAINUI_CONFIG_DIR="$HOME/.config/trainui-trackside"
TRAINUI_CONFIG_FILE="$TRAINUI_CONFIG_DIR/config.json"
POWER_SCHEDULE_CONFIG="/etc/trainui-trackside/power-schedule.json"
SCHEDULE_SLEEP_MARKER="/run/trainui-trackside/display-sleep-v2"
LOG_FILE="$APP_DIR/trainui-trackside.log"
DEPENDENCY_STAMP="$VENV_DIR/.trainui-trackside-requirements.sha256"
MAIN_SOURCE_STAMP="$VENV_DIR/.trainui-trackside-main.sha256"
...
RUNTIME_SPARSE_PATHS=(
    "/.gitignore"
    "/Train UI XL/Train UI XL Trackside/timertest.py"
    "/Train UI XL/Train UI XL Trackside/requirements.txt"
    "/Train UI XL/Train UI XL Trackside/installer/"
)
...
if [ -f "$CORE_SETUP_STAMP" ] || {
    ...
    [ -x "$RUNNER" ] &&
    [ -f "$AUTOSTART_DIR/trainui-trackside.desktop" ]
}; then
...
if [ -d "$REPO_DIR/.git" ]; then
    ...
    if checkout_changes="$(git -C "$REPO_DIR" status --porcelain --untracked-files=all)"; then
        checkout_changes="$(
            printf '%s\n' "$checkout_changes" |
                grep -vE '^\?\? (run_trainui\.sh|trainui-trackside\.log|\.trainui-trackside\.lock|\.venv/)' || true
        )"
    else
...
if command -v nmcli >/dev/null 2>&1 && \
   ! grep -qx 'wifi.powersave=2' /etc/NetworkManager/conf.d/90-trainui-trackside-wifi.conf \
       2>/dev/null; then
    ensure_sudo
    sudo install -d -m 0755 /etc/NetworkManager/conf.d
    sudo tee /etc/NetworkManager/conf.d/90-trainui-trackside-wifi.conf >/dev/null <<'EOF'
[connection]
wifi.powersave=2
EOF
fi
...
if [ ! -f "$APP_DIR/installer/connectivity-watchdog.sh" ] || \
   ...
   [ ! -f "$APP_DIR/installer/systemd/trainui-trackside-wifi-setup.service" ]; then
...
if install_if_changed \
    ...
    /usr/local/sbin/trainui-trackside-connectivity 0755; then
    wifi_files_changed=true
fi
if install_if_changed \
    "$APP_DIR/installer/wifi_setup.py" \
    /usr/local/sbin/trainui-trackside-wifi-setup 0755; then
    wifi_files_changed=true
fi
for wifi_unit in \
    trainui-trackside-connectivity.service \
    trainui-trackside-connectivity.timer \
    trainui-trackside-wifi-setup.service; do
    if install_if_changed \
...
if [ ! -f "$APP_DIR/installer/power_schedule.py" ] || \
   ...
   [ ! -f "$APP_DIR/installer/systemd/trainui-trackside-sleep.service" ] || \
   [ ! -f "$APP_DIR/installer/systemd/trainui-trackside-sleep.timer" ] || \
   [ ! -f "$APP_DIR/installer/systemd/trainui-trackside-wake.service" ] || \
   [ ! -f "$APP_DIR/installer/systemd/trainui-trackside-wake.timer" ] || \
   [ ! -f "$APP_DIR/installer/systemd/trainui-trackside-schedule-sync.service" ]; then
...
if install_if_changed \
    ...
    /usr/local/bin/trainui-trackside-schedule 0755; then
    :
fi
if install_if_changed \
    "$APP_DIR/installer/display-power.sh" \
    /usr/local/sbin/trainui-trackside-display-power 0755; then
    :
fi
for schedule_unit in \
    trainui-trackside-sleep.service \
    trainui-trackside-sleep.timer \
    trainui-trackside-wake.service \
    trainui-trackside-wake.timer \
    trainui-trackside-schedule-sync.service; do
    if install_if_changed \
...
if command -v nmcli >/dev/null 2>&1 && \
   nmcli -t -f DEVICE,TYPE device status 2>/dev/null | grep ':wifi$' >/dev/null; then
    if ! systemctl is-enabled --quiet trainui-trackside-wifi-setup.service 2>/dev/null || \
       systemctl is-enabled --quiet trainui-trackside-connectivity.timer 2>/dev/null; then
        ...
        ensure_sudo
        if ! sudo /usr/local/sbin/trainui-trackside-wifi-setup init >/dev/null; then
            fail "The NetworkManager Wi-Fi setup service could not be initialized."
        fi
        sudo systemctl disable --now trainui-trackside-connectivity.timer 2>/dev/null || true
        sudo systemctl enable --now trainui-trackside-wifi-setup.service
    elif [ "$wifi_files_changed" = true ] || \
         ! systemctl is-active --quiet trainui-trackside-wifi-setup.service 2>/dev/null; then
        ensure_sudo
        sudo systemctl restart trainui-trackside-wifi-setup.service
    fi
    ...
    if systemctl is-enabled --quiet trainui-trackside-wifi-setup.service 2>/dev/null || \
       ! systemctl is-enabled --quiet trainui-trackside-connectivity.timer 2>/dev/null; then
        ensure_sudo
        sudo systemctl disable --now trainui-trackside-wifi-setup.service 2>/dev/null || true
        sudo systemctl enable --now trainui-trackside-connectivity.timer
    fi
...
HEARTBEAT_FILE="\${XDG_RUNTIME_DIR:-/tmp}/trainui-trackside-\${UID}.heartbeat"
...
exec 9>"\$APP_DIR/.trainui-trackside.lock"
flock -n 9 || exit 0
...
if [ -t 1 ] && [ -r /dev/tty ]; then
    ...
    /usr/local/bin/trainui-trackside-schedule \
        --initial \
        --owner "$USER" \
        --home "$HOME" </dev/tty
    ...
    echo "Keeping the existing daily display setting. Run trainui-trackside-schedule to change it."
else
    # A non-interactive install cannot safely guess the owner's desired times.
    sudo /usr/local/bin/trainui-trackside-schedule \
        --disable \
        --owner "$USER" \
        --home "$HOME"
fi
...
cat > "$AUTOSTART_DIR/trainui-trackside.desktop" <<EOF
[Desktop Entry]
Type=Application
Version=1.0
Name=TrainUI
...
sed -i '/# TRAINUI TRACKSIDE START/,/# TRAINUI TRACKSIDE END/d' "$LABWC_DIR/autostart"

cat >> "$LABWC_DIR/autostart" <<EOF
# TRAINUI TRACKSIDE START
"$RUNNER" &
# TRAINUI TRACKSIDE END
EOF
...
if [ ! -f "$APP_DIR/installer/systemd/90-trainui-trackside-runtime-watchdog.conf" ]; then
...
sudo install -m 0644 \
    "$APP_DIR/installer/systemd/90-trainui-trackside-runtime-watchdog.conf" \
    /etc/systemd/system.conf.d/90-trainui-trackside-runtime-watchdog.conf
...
echo "Daily display sleep: run trainui-trackside-schedule to enable, disable, or change it."
...
sudo reboot
    ...
    echo "Daily display sleep: run trainui-trackside-schedule to change it later."
    echo "Runtime log: $LOG_FILE"
