...
The installer places this file at /usr/local/sbin/trainui-trackside-wifi-setup and runs
...
CONFIG_PATH = Path(os.environ.get("TRAINUI_NETWORK_CONFIG", "/etc/trainui-trackside/network.json"))
LOCK_PATH = Path(os.environ.get("TRAINUI_NETWORK_LOCK", "/run/trainui-trackside-network.lock"))
HOTSPOT_PROFILE = "TrainUI-Trackside-Setup"
HOTSPOT_SSID = "TrainUI Trackside"
...
class NetworkManager:
    ...
    def _password_file(self, setting: str, secret: str):
        handle = tempfile.NamedTemporaryFile(
            mode="w",
            encoding="utf-8",
            prefix="trainui-trackside-secret-",
            dir="/run",
            delete=False,
        )
        try:
    ...
    def queue_connection(self, ssid: str, password: str) -> bool:
        ...
        threading.Thread(target=worker, name="trainui-trackside-wifi-connect", daemon=True).start()
        return True
    ...
    def sync(self, mode: str) -> None:
        if mode == "hotspot" and self.server is None:
            ...
            self.thread = threading.Thread(
                ...
                name="trainui-trackside-wifi-portal",
                daemon=True,
            )
            self.thread.start()
